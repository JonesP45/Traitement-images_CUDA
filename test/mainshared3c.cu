...
__global__ void blur_and_edge_detect(const unsigned char * rgb_in, unsigned char * rgb_out, int rows, int cols) {
...
__global__ void edge_detect_and_blur(const unsigned char * rgb_in, unsigned char * rgb_out, int rows, int cols) {
    auto col = blockIdx.x * (blockDim.x-2) + threadIdx.x; //pos de la couleur sur x
    auto row = blockIdx.y * (blockDim.y-2) + threadIdx.y; //pos de la couleur sur y
    auto rgb = threadIdx.z;
    ...
    if( li > 0 && li < (blockDim.x - 1) && lj > 0 && lj < (blockDim.y - 1) && row >= 1 && row < rows - 1 && col >= 1 && col < cols - 1)
    {
        ...
        int somme = (hg + h + hd + g + c + d + bg + b + bd) / 9;

        if (3 * (row * cols + col) + rgb > 6910000)
            printf("%u\n", (3 * (row * cols + col) + rgb));
        rgb_out[3 * (row * cols + col) + rgb] = somme;
    }
}
...
int main()
{
    ...
    dim3 block( 17, 20, 3); //nb de thread, max 1024
