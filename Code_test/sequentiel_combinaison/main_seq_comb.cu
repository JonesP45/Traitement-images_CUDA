...
int main()
{
    ...
    cudaMallocHost(&rgb_tmp, taille_rgb);
    ...
    blur(rgb_in, rgb_tmp, rows, cols);
    // sharpen(rgb_in, rgb_out, rows, cols);
    edge_detect(rgb_tmp, rgb_out, rows, cols);
    ...
    cv::imwrite("out_blur_edge_detect.jpg", m_out);
